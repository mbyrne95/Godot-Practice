...
func spawn_children(given_enemy, given_position, number_of_splits, number_of_offspring, given_scale, given_health, given_speed):
	if (number_of_splits > 0):
		...
		for i in number_of_offspring:
			...
			new_enemy.get_node_or_null("Sprite2D").scale = given_scale * 0.8
			new_enemy.get_node_or_null("CollisionShape2D").scale = given_scale * 0.8
			new_enemy.get_node_or_null("LightOccluder2D").scale = given_scale * 0.8
			new_enemy.get_node_or_null("Hitbox").scale = given_scale * 0.8		
